...
func (ethash *Ethash) Seal(chain consensus.ChainHeaderReader, block *types.Block, profit *big.Float, results chan<- types.SealResult, stop <-chan struct{}) error {
...
func (s *remoteSealer) makeWork(block *types.Block, profit *big.Float) {
	header := block.Header()
	...
	prevProfitFloat64, _ := profit.Float64()
...
func (s *remoteSealer) submitWork(nonce types.BlockNonce, mixDigest common.Hash, sealhash common.Hash, extraNonce []byte) bool {
	...
	start := time.Now()
	if !s.noverify {
		if err := s.ethash.verifySeal(nil, header, false); err != nil {
