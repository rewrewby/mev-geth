...
// localConsole starts a new gexp node, attaching a JavaScript console to it at the
// same time.
...
// for "gexp attach" and "gexp monitor" with no argument.
func dialRPC(endpoint string) (*rpc.Client, error) {
	if endpoint == "" {
		endpoint = node.DefaultIPCEndpoint(clientIdentifier)
	} else if strings.HasPrefix(endpoint, "rpc:") || strings.HasPrefix(endpoint, "ipc:") {
		// Backwards compatibility with gexp < 1.5 which required
		// these prefixes.
		endpoint = endpoint[4:]
	}
