...
var (
	...
	app = utils.NewApp(gitCommit, gitDate, "the go-expanse command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-2020 The go-expanse Authors & the go-expanse Authors"
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	if ctx.GlobalInt(utils.LightLegacyServFlag.Name) > 0 || ctx.GlobalInt(utils.LightServeFlag.Name) > 0 {
		...
		if err := stack.Service(&ethService); err != nil {
			utils.Fatalf("Failed to retrieve expanse service: %v", err)
		}
	...
	if ctx.GlobalString(utils.SyncModeFlag.Name) == "light" {
		...
		if err := stack.Service(&lesService); err != nil {
			utils.Fatalf("Failed to retrieve light expanse service: %v", err)
		}
